...
def show(val):
    global equation
    equation+=val
    result_space.config(text=equation)
...
def calculate():
    ...
    if equation !="":
        try:
            ...
            result="ERROR"
            equation=""
    result_space.config(text=result)
            

result_space = Label(root,width=25,height=2,text="",font=("arial",30))
result_space.pack()
...
Button(root,text ="=",width=5,height=3,font=("ariel",30,"bold"),bd=1,fg="#fff",bg="#fc9803",command=lambda:calculate()).place(x=430,y=400)
